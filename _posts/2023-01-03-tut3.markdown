---
layout: post
title:  "Tutorial 3: Cart-Pole and LQR with MATLAB"
#date:   2023-01-03 19:32:00 +0200
...
$$\dot{\mathbf{r}}_1 = \begin{bmatrix}\dot{x}\\0\end{bmatrix}, \hspace{1cm} \dot{\mathbf{r}}_2=\dot{\mathbf{r}}_1+\ell\dot{\theta}\begin{bmatrix}\cos(\theta)\\\sin(\theta)\end{bmatrix}, \hspace{1cm}\ddot{\mathbf{r}}_1 = \begin{bmatrix}\ddot{x}\\0\end{bmatrix}, \hspace{1cm} \ddot{\mathbf{r}}_2=\ddot{\mathbf{r}}_1 + \ell\ddot{\theta}\begin{bmatrix}\cos(\theta)\\\sin(\theta)\end{bmatrix} + \ell\dot{\theta}^2\begin{bmatrix}-\sin(\theta)\\\cos(\theta)\end{bmatrix}$$
...
To avoid that we use the symbolic toolbox of MATLAB. The following section shows MATLAB code to linearize the system for state-feedback control
...
There are two functions in this snippet: one for the cart-pole model and one for animation. Note that you should place these in the bottom of script.
{% highlight MATLAB %}
